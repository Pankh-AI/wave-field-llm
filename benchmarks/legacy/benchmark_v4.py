"""
V4.1 Linear-Wave Benchmark
===========================
Tests V4.1 linear-wave attention (φ(K)⊙V deposit, φ(Q)⊙gathered read).

Configs:
A) V4.1 Wave (field=1024) — linear-wave, original field size
B) V4.1 Wave (field=512)  — linear-wave, field_size = seq_len
C) V4.1 Wave+Local (f=512, w=64) — linear-wave + near-field

Reference: Standard Transformer PPL ~459, Acc ~11.9% (already benchmarked 4x).
Same training setup: BPE 8K, 512 context, ~7M params, 5M token budget.
"""
...
def main():
    print("=" * 72)
    print("  V4.1 LINEAR-WAVE BENCHMARK")
    print("  phi(K)*V deposit, phi(Q)*gathered read")
    print("=" * 72)
    ...
    # Single long run: V4.1 with 15M tokens to see if breakthrough curve continues
    total_token_budget = 15_000_000

    configs = [
        {
            'name': 'A) V4.1 Wave 15M tokens (f=1024)',
            'type': 'wave',
            'field_size': 1024,
            'local_window': 0,
        },
    ...
    for cfg in configs:
        if cfg['type'] == 'standard':
            ...
            model = WaveFieldTransformer(
                vocab_size=vocab_size, embedding_dim=embedding_dim,
                num_layers=num_layers, num_heads=num_heads,
                ffn_dim=ffn_dim, field_size=cfg['field_size'],
        ...
        try:
            ...
            if cfg['type'] == 'wave':
                result['field_size'] = cfg['field_size']
    ...
    # ============================================================
    print(f"\n{'='*72}")
    print("  RESULTS: V4.1 LINEAR-WAVE BENCHMARK")
    print(f"{'='*72}")
    ...
    results_path = os.path.join(results_dir, 'v4_benchmark.json')
    ...
    wave_results = [r for r in all_results
                    if 'V4.1' in r.get('run_name', '') and isinstance(r.get('best_ppl'), (int, float))]
    if std and wave_results:
        best_wave = min(wave_results, key=lambda r: r['best_ppl'])
        gap = (best_wave['best_ppl'] - std['best_ppl']) / std['best_ppl'] * 100
        print(f"  Standard PPL:     {std['best_ppl']:.1f}")
        print(f"  Best V4.1 PPL:    {best_wave['best_ppl']:.1f} ({best_wave['run_name']})")
        print(f"  Gap:              {gap:+.1f}%")
        print(f"  V3.6 was:         PPL ~1275 | V4.0 was: PPL ~1355")
        if best_wave['best_ppl'] <= 600:
            print(f"  TARGET MET: PPL <= 600!")
        else:
            print(f"  Target: PPL <= 600 ({best_wave['best_ppl'] - 600:.0f} to go)")
